const Jlog = i => console.log(JSON.stringify(i, null, 2));

const str = `suvtxzq (242) -> tdoxrnb, oanxgk
smjsfux (7)
oanxgk (68)
tvvfszm (66)
...
function getNodeDepth({branches}, d = 0) {
  if (branches.length === 0) return d;
  return Math.max(...branches.map(n => getNodeDepth(n, d + 1)));
}

function getNodeWeight({weight, branches}) {
  return (
    weight + branches.map(n => getNodeWeight(n)).reduce((a, b) => a + b, 0)
  );
}

function isBalanced({branches}) {
  const ws = branches.map(n => getNodeWeight(n));
  return Math.max(...ws) === Math.min(...ws);
}

const nws = str.split('\n').map(l => ({
  name: l.match(/[a-z]+/)[0],
  weight: Number(l.match(/[0-9]+/)[0]),
  branches: []
}));
...
const bl = str
  .split('\n')
  .filter(s => s.includes('->'))
  .map(l => [l.match(/[a-z]+/)[0], l.split('->')[1]])
  .map(([n, l]) => [n, l.split(',').map(b => b.trim())]);

class Tree {
  constructor(nodeList) {
    this.nodeList = nodeList
      .map(({name, ...etc}) => ({[name]: {name, ...etc}}))
      .reduce((a, b) => Object.assign({}, a, b));
  }

  addBranch(node, branches) {
    const bNodes = branches.map(n => this.nodeList[n]);
    this.nodeList[node].branches.push(...bNodes);
  }
}

const tr = new Tree(nws);
bl.forEach(([node, branches]) => tr.addBranch(node, branches));

function balanceSearch(nodeList, ib = 'gmcrj') {
  // check children, find the unbalanced one
  const [newBr] = nodeList[ib].branches.filter(b => !isBalanced(b));
  // if all are balanced, current one is at fault
  if (!newBr) return nodeList[ib].branches.map(n => [n.name, getNodeWeight(n)]);
  return balanceSearch(nodeList, newBr.name);
}

Jlog(balanceSearch(tr.nodeList));
