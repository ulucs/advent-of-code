...
defmodule Day14 do
  def add_recipe {recipe_list, i1, i2, recipe_count} do
    ...
    with [some] <- any
    do {insert(recipe_list, recipe_count+1, some), i1+p1+1, i2+p2+1, recipe_count+2}
    else _ -> {recipe_list, i1+p1+1, i2+p2+1, recipe_count+1}
    end    
  end
  ...
  def gold input do
    ...
    |> Enum.reduce_while({%{0 => 3, 1 => 7}, 0, 1, 2}, fn _, acc ->
      ...
      {map, _, _, msz} = recipes
      if(msz < 10, do: {:cont, recipes}, else: cond do
