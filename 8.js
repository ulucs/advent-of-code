const Jlog = i => console.log(JSON.stringify(i));

const code = `b inc 580 if bkd > -1
v inc 491 if z >= -1
...
function lineParser(line = '') {
  const [action, pre] = line.split('if').map(s => s.trim());
  const [reg, op, nu] = pre.split(' ');
  if (ops[op](registers[reg], Number(nu))) {
    const [re2, op2, nu2] = action.split(' ');
    switch (op2) {
      case 'inc':
        registers[re2] += Number(nu2);
        if (registers[re2] > max) max = registers[re2];
        break;
      case 'dec':
        registers[re2] -= Number(nu2);
        break;
...
Jlog(Object.values(registers).sort());
console.log(max);
